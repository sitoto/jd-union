...
module OmniAuth::Strategies
  class JD < OmniAuth::Strategies::OAuth2
    option :client_options, ::JD::Union.gateways
    option :jd_user_role, nil
    ...
    def raw_info
      @rawinfo ||= (!!options.jd_user_role ? fetch_raw_info : fetch_raw_info_from_params)
    end

    private
    def raw_info_method
      ["jd", "user", "#{options.jd_user_role}", "get"].uniq.join(".")
    end
    ...
    def raw_info_fields
      ...
      }[options.jd_user_role.to_s]
    end

    def fetch_raw_info
      service = ::JD::Union::Service.new(options.client_id, options.client_secret, :session => @access_token.token)
      res = service.get(raw_info_method, :fields => raw_info_fields)
      res.error? ? {} : res.body.user
    end

    def fetch_raw_info_from_params
      attrs = [["user_id", "jd_user_id"],["nick", "jd_user_nick"]]
...
::OmniAuth.config.add_camelization 'jd', 'JD'
